...
var Client = function (host, port, logger) {
    ...
    var d = dnode(function (remote, connection) {
        ...
        this.submit = function (itemId, callback) {
            if (state == STATES.SHUTDOWN) {
                logger.log('info', 'Client %s : can\'t execute job in SHUTDOWN state. Reject', id);
                remote.reject(itemId, function () {
                    callback(itemId);
                });
            } else {
                logger.log('info', 'Client %s : Accept', id);
                remote.accept(itemId, function () {
                    callback(itemId);
                });
            }
        };
        ...
        this.shutdown = function (callback) {
            ...
            shutdownWaitHandle = setInterval(function () {
                ...
                if (runningJobs == 0 && state == STATES.SHUTDOWN) {
